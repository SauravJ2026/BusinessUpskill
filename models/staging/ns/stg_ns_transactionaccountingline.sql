...
renamed AS (
    ...
    SELECT
        MD5(COALESCE(CAST("TRANSACTION" AS VARCHAR),'~') || '|' || COALESCE(CAST(ACCOUNTINGBOOK AS VARCHAR),'~') || '|' || COALESCE(CAST(TRANSACTIONLINE AS VARCHAR),'~') || '|' || COALESCE(CAST("ACCOUNT" AS VARCHAR),'~') || '|' || 'NS') AS SURROGATE_KEY,
        MD5(COALESCE(CAST("TRANSACTION" AS VARCHAR),'~') || '|' || COALESCE(CAST(ACCOUNTINGBOOK AS VARCHAR),'~') || '|' || COALESCE(CAST(TRANSACTIONLINE AS VARCHAR),'~') || '|' || COALESCE(CAST("ACCOUNT" AS VARCHAR),'~')) AS RECORD_HASH,
